defmodule Paddle.SubscriptionFlowsTest do
  ...
  test "immediate upgrade processes immediately and returns updated subscription", %{client: client} do
    assert {:ok, %Subscription{} = sub} = Paddle.Subscriptions.update(
             client,
             "sub_mock123",
  ...
  test "scheduled downgrade populates scheduled_change struct on returned subscription", %{client: client} do
    assert {:ok, %Subscription{} = sub} = Paddle.Subscriptions.update(
             client,
             "sub_mock123",
