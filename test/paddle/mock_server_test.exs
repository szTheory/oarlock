defmodule Paddle.MockServerTest do
  ...
  test "source gates Plug and Bandit behind optional dependency checks" do
    source = File.read!("lib/paddle/mock_server.ex")

    assert source =~ "Code.ensure_loaded?(Plug.Router)"
    assert source =~ "Code.ensure_loaded?(Plug.Parsers)"
    assert source =~ "Code.ensure_loaded?(Bandit)"
    assert source =~ "defmodule Router"
  end
  ...
  test "POST /customers returns a mocked customer payload", %{client: client} do
    assert {:ok, %Paddle.Customer{} = customer} =
             Paddle.Customers.create(client, %{email: "test@example.com", name: "Test"})
  ...
  test "PATCH /customers/:id returns a mocked customer payload", %{client: client} do
    assert {:ok, %Paddle.Customer{} = customer} =
             Paddle.Customers.update(client, "ctm_dynamic_789", %{name: "Updated"})

    assert customer.id == "ctm_dynamic_789"
  end

  test "POST /transactions returns a mocked transaction payload", %{client: client} do
    assert {:ok, %Paddle.Transaction{} = txn} =
  ...
  test "POST /customers/:id/portal-sessions dynamically injects the customer_id", %{
    client: client
  } do
    assert {:ok, %Paddle.PortalSession{} = session} =
             Paddle.PortalSessions.create(client, %{customer_id: "ctm_portal_test"})
  ...
  test "PATCH /subscriptions/:id returns a mocked subscription updated payload for immediate proration",
       %{client: client} do
    assert {:ok, %Paddle.Subscription{} = sub} =
             Paddle.Subscriptions.update(client, "sub_mock456", %{
  ...
  test "PATCH /subscriptions/:id returns a mocked scheduled change payload for next billing period",
       %{client: client} do
    assert {:ok, %Paddle.Subscription{} = sub} =
             Paddle.Subscriptions.update(client, "sub_mock456", %{
    ...
    assert %Paddle.Subscription.ScheduledChange{
             action: "pause",
